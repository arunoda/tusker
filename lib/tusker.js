...
function Tusker(commandClient, watchingClient) {
    ...
    this.close = function close(taskName, info, options, callback) {
        ...
        var keys = [
            getLockHashName(taskName), getClosedHashName(), 
            getReleasedListName()
        ];
            ...
            if(err) {
                ...
                try {
                    ...
                    if(info && info.metadata) {
                        ...
                        callback(new Error("Internal Error - Invalid JSON Task Info"));
                    }
                } catch(ex) {
                    callback(new Error("Internal Error - Invalid JSON String"));
                }
            }
        });
