...
function Tusker(commandClient, watchingClient) {
    ...
    this.timeoutLocks = function timeout(millis, callback) {
    ...
    function startWatching (callback) {
        ...
        function processCompleted(err, callback) {
            ...
            if(err) {
                ...
                var keys = [getClosedHashName(), getProcessingHashName(), getCompletedListName()];
                var args = [info.metadata.task, Date.now()];
...
exports._getCompletedListName = getCompletedListName;